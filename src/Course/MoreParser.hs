...
sepby1 ::
  ...
  -> Parser (List a)
sepby1 p s =
  p >>= (<$> ((s >> sepby p s) ||| valueParser Nil)) . (:.)
{-
sepby1 p s = do
  x <- p
  xs <- (s >> sepby p s) ||| valueParser Nil
  return $ x :. xs
-}
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma c1 c2 =
  betweenCharTok c1 c2 . flip sepby (charTok ',')
