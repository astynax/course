...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat = (`eval` S.empty) . findM isRepeat
...
distinct ::
  ...
  List a
  -> List a
distinct = (`eval` S.empty) . filtering ((not <$>) . isRepeat)
...
isHappy ::
  ...
  -> Bool
isHappy x =
  case eval (findM isIt (produce step x)) S.empty of
    Full y -> y == 1
    _      -> False
  where
    isIt 1 = pure True
    ...
    step = toInteger . sum . map ((\x -> x * x) . digitToInt) . show'


isRepeat :: Ord a => a -> State (S.Set a) Bool
isRepeat x = get >>= \alreadySeen ->
  if S.member x alreadySeen
  then pure True
  else put (S.insert x alreadySeen)
       >> pure False
