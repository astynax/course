...
toList ::
  ListZipper a
  -> List a
toList (ListZipper Nil x xs) =
  x :. xs
toList (ListZipper (y :. ys) x xs) =
  toList . ListZipper ys y $ x :. xs
...
findLeft p (ListZipper l x r) =
  case break p l of
    (xs, x' :. l') ->
      IsZ $ ListZipper l' x' (reverse xs ++ x :. r)
    _ ->
      IsNotZ
...
findRight p (ListZipper l x r) =
  case break p r of
    (xs, x' :. r') ->
      IsZ $ ListZipper (reverse xs ++ x :. l) x' r'
    _ ->
      IsNotZ
...
moveRightN' ::
  ...
  -> Either Int (ListZipper a)
moveRightN' =
  move moveRight moveLeftN'

move ::
  (ListZipper a -> MaybeListZipper a)
  -> (Int -> ListZipper a -> Either Int (ListZipper a))
  -> Int -> ListZipper a -> Either Int (ListZipper a)
move shift opposite n z
  | n < 0 = opposite (negate n) z
  | otherwise = try 0 n z
  where
    try cnt nn zz
      | nn == 0 = Right zz
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight y (ListZipper l x r) =
  ListZipper l y (x :. r)
...
instance Extend ListZipper where
  (<<=) f z =
    f <$> ListZipper (use moveLeft z) z (use moveRight z)
    where
      use = unfoldr . with . (toOptional .)
      with f' z' = (\x -> (x, x)) <$> f' z'
