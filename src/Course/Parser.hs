...
list1 ::
  ...
  -> Parser (List a)
list1 p =
  p `flbindParser` (\x -> mapParser (x :.) (list p))
...
natural ::
  Parser Int
natural =
  list1 digit `flbindParser` (\xs -> case read xs of
                                 Full x ->
                                   valueParser x
                                 _ ->
                                   failed
...
sequenceParser =
  foldRight
  (\p pp -> p `flbindParser` (\x -> mapParser (x :.) pp))
  (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser =
  upper `flbindParser` (\x -> mapParser (x :.) (list lower))
    ...
    (\name' ->
      ...
      (\surname' ->
        ...
        (\smoker' ->
          ...
          (\phone' ->
            valueParser
            (Person age' name' surname' smoker' phone')
          )
        )
      )
    )
